...
int main(int argc, char const *argv[])
{
    ...
    {	     	
    	...
    	{	           
            ...
            {   
                Book iTempBook;
                iTempBook.number = ptrsBook[x].number;
                strcpy(iTempBook.word,fnIgnoreBackSlashN(ptrsBook[x].word,strlen(ptrsBook[x].word)));
                write(iFifo,&iTempBook,sizeof(Book));
            }
            close(pipefd[0]);
    ...
    Book iTempStruct[iTempCount];
    for (i = 0; i < iTempCount; ++i)
    {           
        read(iFifo,&iTempStruct[i],sizeof(Book));     
    }
    int iNumber = 0;
    int iTotalWordsNumber = 0;
    Book*ptrsTemp = fnCombineAllOfWords(iTempStruct,iTempCount,&iNumber);
    for (i = 0; i < iNumber; ++i)
    {

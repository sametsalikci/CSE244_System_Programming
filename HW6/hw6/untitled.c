    ...
    while ((direntp = readdir(dirp)) != NULL) {
        ...
        if (fnIsDirectory(direntp->d_name)){
            ...
            if (childpid == -1) {
                ...
                printf("newPathilk:%s\n",root);
                //strcat(newTempPath,newRoot);        
                while ((closedir(dirp) == -1) && (errno == EINTR));
                if ((dirpp = opendir(root)) == NULL) {
                    fprintf (stdout, "Failed to open directory \"%s\" : %s\n",
                             direntp->d_name, strerror(errno));
                    return 1;
                }
                    ...
                    while ((direntpp = readdir(dirpp)) != NULL) {
                        char   newTempPath[MAX_PATH] = "";
    
                        if (direntpp->d_name[0]=='.') continue;
                        empty=0;
                        if( direntpp->d_name[strlen(direntpp->d_name)-1] == '~'  ) continue;

                        //printf("samet %d\n",++a);
                        if (!(fnIsDirectory(direntpp->d_name))){
            
                            strcat(newTempPath, root);
                            strcat(newTempPath, "/");
                            strcat(newTempPath, direntpp->d_name);
                            printf("file:%s\n",newTempPath);
                            // FILE*pfLogFileOutput;
                            // if(( pfLogFileOutput = fopen(newTempPath,"w")) == NULL){
                            //     perror("Failed to open file for writting!!\n");
                            //     return 0;
                            // }
                            // fclose(pfLogFileOutput);       
                        } // Directory icindeki okunan name'in filename olma durumu
                    }
                    while ((closedir(dirpp) == -1) && (errno == EINTR)); // force to close directory
                    //while ((closedir(dirp) == -1) && (errno == EINTR));
               /////////////////////////////////////////
                printf("pathson:%s\n",root);         
            ...
            if (path != ""){
                //printf("%s\n",path);
                //if( direntpp->d_name[strlen(direntpp->d_name)-1] != '~'  )
